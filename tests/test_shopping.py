...
def test_shopping(driver):
    ...
    elem.clear()
    elem.send_keys("iphone 11 pro max")
    elem.send_keys(Keys.RETURN)
    assert "No results found." not in driver.page_source

    """enumeration elements"""
    customer_reviews = {}
    WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.XPATH, '//div[contains(@data-component-type, "s-search-result")]')))
    all_products = driver.find_elements(By.XPATH, '//div[contains(@data-component-type, "s-search-result")]')
    for element in all_products:
        number_reviews = element.find_elements(By.CSS_SELECTOR, 'span')
        for review in number_reviews:
            if review.get_attribute("aria-label") is not None and "star" not in review.get_attribute("aria-label"):
                reviews = review.get_attribute("aria-label").replace(',', '')
                if reviews.isdigit():
                    customer_reviews[element] = int(reviews)   # saving product in dict as key
    ...
    while True:
        ...
        try:
            product_price_best_buy = max_customer_reviews.find_element(By.XPATH, './/div[contains(@class, "priceView-customer-price")]')
            best_price = \
